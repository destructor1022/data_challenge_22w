...
df = read.csv("C:/Users/sauna/Documents/Documents/Dartmouth/Clubs/DALI Data/wiid.csv")
...
ggplot(data = gini.by.cont, aes(x = factor(year), y = gini_reported, color = region_un)) +       
  ...
  labs(title="Average Gini by Continent Over Time") +
  xlab("Year") +
  ylab("Gini")
...
ggplot(data = earnings.by.cont, aes(x = factor(year), y = gdp_ppp_pc_usd2011, color = region_un)) +       
  ...
  geom_point() + 
  scale_x_discrete(guide = guide_axis(check.overlap = TRUE)) +
  labs(title="Average Earnings by Continent Over Time") +
  xlab("Year") +
